...
char Serial_RxPacket_USART1[PACKET_LEN];				
uint8_t Serial_RxFlag_USART1=0;	
char Serial_RxPacket_USART2[PACKET_LEN];				
uint8_t Serial_RxFlag_USART2=0;	
char Serial_RxPacket_USART3[PACKET_LEN];	
uint8_t Serial_RxFlag_USART3=0;		
char Serial_RxPacket_UART4[PACKET_LEN];	
uint8_t Serial_RxFlag_UART4=0;	
char Serial_RxPacket_UART5[PACKET_LEN];	
uint8_t Serial_RxFlag_UART5=0;

/* init function*/
void Serial_USARTx_Init(void){
	Serial_USART1_Init(115200);
	Serial_USART2_Init(115200);
	Serial_USART3_Init(9600);
	Serial_UART4_Init(460800);
	Serial_UART5_Init(115200);
}
...
void UART5_IRQHandler(void){
	...
	if (USART_GetITStatus(UART5, USART_IT_RXNE) == SET){
		...
		if (RxState == 0){
			if (Serial_RxFlag_UART5 == 0  && RxData=='('){
				RxState = 1;			
				pRxPacket = 0;
				Serial_RxPacket_UART5[pRxPacket] = RxData;
				pRxPacket ++;		
			}
		}
		else if (RxState == 1){
			if (RxData == ')'){
				...
				Serial_RxPacket_UART5[pRxPacket] = RxData;
				pRxPacket ++;		
				Serial_RxPacket_UART5[pRxPacket] = '\0';
				Serial_RxFlag_UART5 = 1;
			}
			else{
				Serial_RxPacket_UART5[pRxPacket] = RxData;		
				pRxPacket ++;			
